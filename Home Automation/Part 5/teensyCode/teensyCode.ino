...
const int secOffPin = 6;
const int secArmedPin = 5;
const int secTriggeredPin = 4;
...
void setup(){
	...
	pinMode(secOffPin, OUTPUT);
	pinMode(secArmedPin, OUTPUT);
	pinMode(secTriggeredPin, OUTPUT);

	//initially assume that the security system is disarmed
	digitalWrite(secOffPin, HIGH);
...
void loop(){
	...
	while(Serial1.available() && (Serial1.peek() != 'd' && Serial1.peek() != 't' && Serial1.peek() != 'l' && Serial1.peek() != 's')){	
		Serial1.read();
	}
	...
	if(Serial1.available() >= 2){
		...
		//handle security system packet
		else if(headerByte == 's'){
			//system has been disabled
			if(payloadByte == SEC_OFF){
				digitalWrite(secOffPin, HIGH);
				digitalWrite(secArmedPin, LOW);
				digitalWrite(secTriggeredPin, LOW);
			}

			//system has been armed
			else if(payloadByte == SEC_ARMED){
				digitalWrite(secOffPin, LOW);
				digitalWrite(secArmedPin, HIGH);
				digitalWrite(secTriggeredPin, LOW);
			}

			//system has been triggered
			else if(payloadByte == SEC_TRIGGERED){
				digitalWrite(secOffPin, LOW);
				digitalWrite(secArmedPin, LOW);
				digitalWrite(secTriggeredPin, HIGH);
			}
		}
